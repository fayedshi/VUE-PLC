...
const submitForm = async () => {
  // # 💡 如果对接真实后端，此处应该发起 Axios HTTP 请求
  await axios.post('/api/granary', form)

  if (modalType.value === 'add') {
    // # 模拟后端【新增】逻辑：校验编号是否重复
    const isExist = granaryList.value.some(item => item.code === form.code)
    if (isExist) {
      alert('❌ 报错：该廒间编号已经存在，请勿重复添加！')
      return
    }
    
    // # 插入新数据
    granaryList.value.push({
      id: Date.now(),
      ...form
    })
    alert('🎉 提示：新廒间添加成功！')
    ...
    // # 模拟后端【修改】逻辑：找到对应的 ID 覆盖数据
    const index = granaryList.value.findIndex(item => item.id === currentEditId)
    if (index !== -1) {
      granaryList.value[index] = { id: currentEditId, ...form }
      alert('📝 提示：廒间信息修改成功！')
    }
  }
  closeModal()
...
const deleteItem = async (id) => {
  if (confirm('⚠️ 警告：确定要删除该廒间吗？删除后关联的温度历史将一同消失！')) {
    // # 💡 对应后端：
    await axios.delete(`/api/granary/${id}`)
...
.btn-add:hover { background: #a3e044; }
...
.custom-table tbody tr:hover {
  background-color: rgba(134, 194, 50, 0.05); /* 鼠标悬停高亮 */
}

/* 美化小部件 */
.bold-text { font-weight: bold; color: #fff; }
.user-badge { background: #474b4f; padding: 4px 8px; border-radius: 4px; font-size: 13px; }
.grain-tag { background: rgba(134, 194, 50, 0.2); color: #86c232; padding: 3px 8px; border-radius: 4px; border: 1px solid rgba(134,194,50,0.3); }
.warning-text { color: #e74c3c; font-weight: bold; }
.empty-text { text-align: center; color: #666; padding: 40px 0; }

/* 操作按钮 */
.action-btns { display: flex; gap: 8px; }
.btn-edit, .btn-delete {
  border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; font-size: 12px; transition: opacity 0.2s;
}
...
.modal-header {
  background: #1a1a1d; padding: 15px 20px;
  display: flex; justify-content: space-between; align-items: center;
  border-bottom: 1px solid #474b4f;
}
.modal-header h3 { margin: 0; color: #86c232; font-size: 16px; }
.close-x { background: transparent; border: none; color: #aaa; font-size: 22px; cursor: pointer; }
...
.form-item label { font-size: 13px; color: #ccc; }
.form-item input, .form-item select {
  background: #1a1a1d; border: 1px solid #474b4f; color: #fff;
  padding: 10px; border-radius: 4px; font-size: 14px; outline: none;
}
...
.btn-cancel { background: #474b4f; color: #fff; border: none; padding: 10px 18px; border-radius: 4px; cursor: pointer; }
.btn-submit { background: #86c232; color: #000; border: none; padding: 10px 18px; border-radius: 4px; cursor: pointer; font-weight: bold; }
