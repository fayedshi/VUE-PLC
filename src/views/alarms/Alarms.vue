  ...
  <div class="grain-alarm-container">
      ...
      <div class="table-wrapper">
        <table class="native-table">
          ...
          <tbody>
            ...
            <tr v-for="item in activeAlarms" :key="item.house_id + item.type" :class="['alarm-row', item.type]">
              <td class="time-cell">{{ item.time }}</td>
              <td><strong>{{ item.house_name }}</strong></td>
              <td>
                <span :class="['badge', item.type === 'PLC_DISCONNECT' ? 'badge-danger' : 'badge-warning']">
    ...
    <div class="card history-card">
      ...
      <div class="pagination">
        ...
        <span class="page-info">第 {{ historyQuery.page }} 页 / 共 {{ Math.ceil(historyTotal / historyQuery.size) || 1 }} 页</span>
        <button :disabled="historyQuery.page * historyQuery.size >= historyTotal" @click="changePage(1)">下一页</button>
      </div>
    </div>
...
const initWebSocket = () => {
  wsClient = new WebSocket('ws://localhost:8088/ws/alarms')

  wsClient.onmessage = (event) => {
    const data = JSON.parse(event.data)

    if (data.event === 'INIT_ACTIVE_ALARMS') {
      activeAlarms.value = data.data
      return
    }
...
/* 报警行高亮背景调色 */
.alarm-row.PLC_DISCONNECT { background-color: #fff1f0; }
.alarm-row.TEMP_HIGH { background-color: #fffbe6; }
...
.toast.danger { border-left-color: #ff4d4f; }
